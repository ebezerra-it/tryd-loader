...
CMD_INFO_RETURN=$(virsh -c qemu:///system dominfo "$VM_NAME")
...
if [ "$VM_STATE" = "running" ] ; then
    ...
    echo "{ \"status\": \"success\", \"message\": \"[VMSTATUS] VM status script was executed successfuly in VM $VM_NAME\", \"vmstatus\": { \"state\": \"$VM_STATE\", \"upseconds\": $VM_UPSECONDS } }"
    exit 0
else
    echo "{ \"status\": \"success\", \"message\": \"[VMSTATUS] VM status script was executed successfuly in VM $VM_NAME\", \"vmstatus\": { \"state\": \"$VM_STATE\" } }"
    exit 0
fi
